...
def decide_next(state: Dict[str, Any]) -> Step:
    """
    Decide next step based on enhanced state.
...
def get_step_recommendation(state: Dict[str, Any]) -> Dict[str, Any]:
    """
    Get detailed recommendation for next step.
...
if __name__ == "__main__":
    ...
    next_step = decide_next(state1)
    recommendation = get_step_recommendation(state1)
    ...
    next_step = decide_next(state2)
    recommendation = get_step_recommendation(state2)
    ...
    next_step = decide_next(state3)
    recommendation = get_step_recommendation(state3)
