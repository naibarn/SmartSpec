...
1. `.smartspec/smartspec.config.json` (if exists - tooling/organization config)
2. `.spec/SPEC_INDEX.json` (canonical; fallback to `SPEC_INDEX.json` root mirror for legacy)
3. Built-in defaults
...
```bash
test -f .spec/SPEC_INDEX.json && echo "EXISTS_CANONICAL" || test -f SPEC_INDEX.json && echo "EXISTS_LEGACY_ROOT" || test -f .smartspec/SPEC_INDEX.json && echo "EXISTS_DEPRECATED" || echo "NOT_EXISTS"
```
...
If NOT_EXISTS:
```
1. Create `.spec/SPEC_INDEX.json` with initial structure (canonical location):
   {
     "version": "5.1",
...
2. Log: "✅ Created .spec/SPEC_INDEX.json (canonical)"
```

**Step 3: Load SPEC_INDEX.json into memory**

If EXISTS (or just created):
- Read `.spec/SPEC_INDEX.json` (or resolved legacy path)
