...
## 12) การตรวจสอบย้อนกลับและความสมบูรณ์ (ต้องมี)

เพื่อให้แน่ใจว่าสามารถตรวจสอบย้อนกลับจากข้อกำหนดไปยังทาสก์ได้อย่างสมบูรณ์ AI agent จะต้องสร้างส่วนต่อไปนี้ และสคริปต์การตรวจสอบจะต้องตรวจสอบความถูกต้อง

### 12.1 เมทริกซ์การตรวจสอบย้อนกลับข้อกำหนด (RTM)

ส่วนใหม่ที่จำเป็นนี้จะจับคู่ข้อกำหนดจาก `spec.md` กับทาสก์ที่นำไปปฏิบัติใน `tasks.md`

#### 12.1.1 เทมเพลตการครอบคลุมข้อกำหนดด้านความปลอดภัย

```markdown
## เมทริกซ์การตรวจสอบย้อนกลับข้อกำหนด

### การครอบคลุมข้อกำหนดด้านความปลอดภัย

| รหัสข้อกำหนด | คำอธิบาย | ทาสก์ที่นำไปปฏิบัติ | สถานะการครอบคลุม |
|---|---|---|---|
| SEC-001 | การแฮชรหัสผ่าน (bcrypt, cost 12) | TSK-AUTH-025 | ✅ สมบูรณ์ |
| SEC-002 | อัลกอริทึม JWT (RS256, JWKS endpoint) | TSK-AUTH-030, TSK-AUTH-031, TSK-AUTH-032 | ✅ สมบูรณ์ |
```

#### 12.1.2 เทมเพลตการครอบคลุมข้อกำหนดเชิงฟังก์ชัน

```markdown
### การครอบคลุมข้อกำหนดเชิงฟังก์ชัน

| รหัส T-ID | คำอธิบาย | รหัส TSK-ID | สถานะการครอบคลุม |
|---|---|---|---|
| T001 | โมเดลผู้ใช้ (Prisma schema) | TSK-AUTH-020 | ✅ สมบูรณ์ |
| T010 | การจัดการโทเค็น JWT (RS256) | TSK-AUTH-030 | ✅ สมบูรณ์ |
```

### 12.2 เทมเพลตรายการตรวจสอบความสมบูรณ์

รายการตรวจสอบนี้จะให้ภาพรวมระดับสูงของการครอบคลุมข้อกำหนด

```markdown
## รายการตรวจสอบความสมบูรณ์

### ข้อกำหนดด้านความปลอดภัย (SEC-XXX)
- [x] SEC-001: การแฮชรหัสผ่าน
- [x] SEC-002: อัลกอริทึม JWT (รวมถึง JWKS endpoint)

### ข้อกำหนดเชิงฟังก์ชันที่สำคัญ
- [x] การลงทะเบียนผู้ใช้พร้อมการยืนยันอีเมล
- [x] JWKS endpoint สำหรับการแจกจ่าย public key
```

### 12.3 การตรวจสอบย้อนกลับแบบย้อนกลับในรายการทาสก์

แต่ละรายการทาสก์จะต้องมีข้อมูลเมตาเพื่อติดตามกลับไปยังข้อกำหนดที่นำไปปฏิบัติ

```markdown
- [ ] **TSK-AUTH-032: นำ JWKS endpoint ไปปฏิบัติ**
  - **นำไปปฏิบัติ:** SEC-002 (อัลกอริทึม JWT - JWKS endpoint)
  - **อ้างอิง T-Reference:** T010 (การจัดการโทเค็น JWT)
  - **เกณฑ์การยอมรับ:**
    - [ ] Endpoint `GET /.well-known/jwks.json` ส่งคืน JWK Set
  - **ตะขอเกี่ยวกับหลักฐาน:**
    - **โค้ด:** `packages/auth-service/src/routes/jwks.route.ts`
    - **การทดสอบ:** `packages/auth-service/tests/e2e/jwks.test.ts`
```

### 12.4 การตรวจสอบที่ปรับปรุงแล้ว

สคริปต์การตรวจสอบได้รับการปรับปรุงเพื่อตรวจสอบการตรวจสอบย้อนกลับ

#### 12.4.1 คำสั่งการตรวจสอบที่ปรับปรุงแล้ว
...
#### 12.4.2 กฎการตรวจสอบที่ปรับปรุงแล้ว

- อาร์กิวเมนต์ `--spec` เป็น **สิ่งจำเป็น**
- สคริปต์จะแยกวิเคราะห์ `spec.md` เพื่อค้นหาข้อกำหนด `SEC-XXX` และการอ้างอิง `T-XXX` ทั้งหมด
- จากนั้นจะตรวจสอบว่าข้อกำหนดและการอ้างอิงทั้งหมดมีอยู่ใน **เมทริกซ์การตรวจสอบย้อนกลับข้อกำหนด**
- การตรวจสอบจะล้มเหลวหากมีข้อกำหนดใดไม่ครอบคลุม
- ผลลัพธ์ทั้งหมด รวมถึงสถิติการครอบคลุม จะต้องรวมอยู่ใน `report.md`
